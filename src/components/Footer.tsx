...
const Footer = ({ brandLabel = "ORIGEN", compact = false }: FooterProps) => {
  return <footer className="bg-primary text-primary-foreground">
      <div className={`container mx-auto px-6 ${compact ? "py-10" : "py-16"}`}>
          ...
          <div className="space-y-6">
            ...
            <nav className="flex flex-col space-y-3">
              <Link to="/historia" className="text-primary-foreground/80 hover:text-secondary transition-colors">
                {brandLabel === "ORIGEN" ? "Sobre Origen" : "Sobre RITMORIGEN"}
              </Link>
        ...
        {/* Línea divisoria */}
        <div className={`border-t border-primary-foreground/20 ${compact ? "mt-8 pt-6" : "mt-12 pt-8"}`}>
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <p className="text-primary-foreground/60 text-sm">
              © 2025 {brandLabel}. Todos los derechos reservados.
            </p>
