/**
 * Tarjeta premium de filtros — 3 grupos:
 *   A) NICHO (segmented buttons + iconos)
 *   B) TIPO (segmented buttons + iconos)
 *   C) PROVINCIA (select estilizado) + toggles + reset
 *
 * Todos los iconos son inline SVG (sin dependencias extra).
 */
...
export const MapFiltersCard: React.FC<MapFiltersCardProps> = ({
  ...
  return (
    <div
      className="hidden lg:block rounded-[22px] p-6 mt-8"
      style={{
    ...
    >
      {/* GROUP A — NICHO */}
      <div className="space-y-2.5 pb-5 border-b" style={{ borderColor: ORIGEN_COLORS.beigeSoft }}>
        <FilterLabel>Nicho</FilterLabel>
        <Segmented options={NICHO_OPTIONS} value={nicho} onChange={onNichoChange} />
      </div>

      {/* GROUP B — TIPO */}
      <div className="space-y-2.5 py-5 border-b" style={{ borderColor: ORIGEN_COLORS.beigeSoft }}>
        <FilterLabel>Tipo</FilterLabel>
        <Segmented options={TIPO_OPTIONS} value={tipo} onChange={onTipoChange} />
      </div>

      {/* GROUP C — PROVINCIA + TOGGLES */}
      <div className="pt-5 flex flex-wrap items-end gap-4">
        <div className="space-y-2.5 min-w-[200px]">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none w-full pl-4 pr-10 py-2.5 text-sm font-semibold rounded-xl border cursor-pointer transition-all"
              style={{
                backgroundColor: ORIGEN_COLORS.cream,
        ...
        <div className="flex flex-wrap gap-2 ml-auto">
          <PillToggle
            ...
            icon={<Star size={15} fill={destacadosPrimero ? ORIGEN_COLORS.cream : 'none'} />}
          ...
          <button
            type="button"
            onClick={onReset}
            className="flex items-center gap-2 px-3.5 py-2 rounded-full border text-xs font-semibold uppercase tracking-wider transition-all hover:opacity-80"
            style={{
